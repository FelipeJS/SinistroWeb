...
<head id="Head1" runat="server">
    ...
    ['SC', 'Santa Catarina'], ['SE', 'Sergipe'], ['SP', 'São Paulo'], ['TO', 'Tocantins']
        ...
        $(document).ready(function () {
            ...
            $("input[name='addReu']").click(function (e) {
                if (document.getElementById('txtReu').value != "")
                    $("#cboReu").append($("<option>").attr("value", "1").attr("selected", "selected").text(document.getElementById('txtReu').value));
                document.getElementById('txtReu').value = '';
            });

            $("input[name='addAutor']").click(function (e) {
                if (!validarCPF(document.getElementById('txtAutor').value)) {
                    if (!validarCNPJ(document.getElementById('txtAutor').value)) {
                ...
                $.ajax({
                    ...
                    url: "pessoa?numeroCPF=" + document.getElementById('txtAutor').value,
                    data: "{}",
                    dataType: 'json',
                    success: function (result) {
                        if (result == '')
                            alert("Pessoa não cadastrada, favor cadastrar!");
                        else {
                            var cpf = result[0];
                            $("#cboAutor").append($("<option>").attr("value", "1").attr("selected", "selected").text(cpf.NR_DOCTO + " - " + cpf.DS_PESSOA));
            ...
            $('#input_adicional_reu, #input_adicional_autor').delegate('a', 'click', function (e) {
                e.preventDefault();
            ...
            .click(function () {
                $.ajax({
                    ...
                    url: "sinistro?judicialDados=" + document.getElementById('FormView1_lblIdSinistro').innerHTML + '$' +
                                              ...
                                              document.getElementById('cboReu').innerHTML.replace(/\$/g, '^').replace(/\@/g, '~') + '$' +
                                              ...
                                              document.getElementById('cboAutor').innerHTML.replace(/\$/g, '^').replace(/\@/g, '~') + '$' +
                                              ...
                                              document.getElementById('txtNovaIndenizacao').value.replace(",", ".") + '$' +
                                              document.getElementById('txtNovoHonorario').value.replace(",", ".") + '$' +
                                              document.getElementById('txtReservaDisponivel').value.replace(",", ".") + '$' +
                                              document.getElementById('txtReservaDisponivelHon').value.replace(",", "."),
                    data: "{}",
                    dataType: 'json',
            ...
            $("#txtDtPagto").datepicker({ minDate: 0 });
            ...
            .click(function () {
                AlterarConta.style.display = 'none';
            });
            ...
            .click(function () {
                ...
                $("#flexDocumentos").flexigrid({
                    ...
                    colModel: [
                        ...
                        { display: 'Nome', name: 'DS_PESSOA', width: 200, sortable: true, align: 'left', process: abrirBeneficiario }
                    ],
                    title: "",
                    useRp: false,
                    showTableToggleBtn: false,
...
<!-- FRAME 0 -->
 ...
 order by t.id_liquidacao"></asp:SqlDataSource>
    ...
    <asp:SqlDataSource ID="dsReservaPendente" runat="server" 
        ConnectionString="<%$ ConnectionStrings:DB1 %>" 
        ProviderName="<%$ ConnectionStrings:DB1.ProviderName %>" 
        SelectCommand="select t.ds_descricao, t.vl_total, 
(select t.vl_total from ctb_vtotalreservaspendente t where t.ds_descricao like 'HONORARIOS' and t.id_sinistro = :idSinistro) as vl_honorario,
(select t.vl_total from ctb_vtotalreservaspendente t where t.ds_descricao like 'INDENIZA%' and t.id_sinistro = :idSinistro) as vl_indenizacao,
(select 
(select t.vl_total from ctb_vtotalreservaspendente t where t.ds_descricao like 'HONORARIOS' and t.id_sinistro = :idSinistro ) 
+ 
(select t.vl_total from ctb_vtotalreservaspendente t where t.ds_descricao like 'INDENIZA%' and t.id_sinistro = :idSinistro )
 from ctb_vtotalreservaspendente t where t.id_sinistro = :idSinistro and rownum = 1) as vl_soma
from ctb_vtotalreservaspendente t 
where t.id_sinistro = :idSinistro and t.vl_total <> 0">
    </asp:SqlDataSource>
    ...
    <legend>Movimento Reserva Indenização</legend>
            ...
            <asp:GridView ID="GridView9" runat="server" DataSourceID="dsReservaMovimento" AutoGenerateColumns="false"  CssClass="DDGridView" RowStyle-CssClass="td" HeaderStyle-CssClass="th" CellPadding="6">
                    <Columns>
                        ...
                        <asp:BoundField DataField="ds_usuario" HeaderText="Nome do Usuário"  />
                    </Columns>
            </asp:GridView>
    ...
    <asp:SqlDataSource ID="dsReservaMovimento" runat="server" ConnectionString="<%$ ConnectionStrings:DB1 %>" ProviderName="<%$ ConnectionStrings:DB1.ProviderName %>" 
                       SelectCommand="select tpmoid fl_evento, to_char(r.dt_cadastro, 'dd/MM/yyyy') data, r.*, ds_usuario
                                       from adm_usuario a right outer join sto_reserva t on (t.id_usuario = a.id_usuario)
                                       inner join VRESERVA_STATUS r on (t.id_reserva = r.id_reserva)
...
-webkit-font-smoothing: antialiased;
    ...
    <p></p>
                            ...
                            <select id="txtMotivoAcao" class="novoSelect" style="width:238px; height:21px; margin-top:0" >
                                <option value="FalhaAtendimento" <%# Eval("DS_MOTIVO_ACAO") %> >Falha Atendimento</option>
                                <option value="NegativaDeSinistro" <%# Eval("DS_MOTIVO_ACAO") %> >Negativa de Sinistro</option>
                                <option value="DivergenciaIndenizacao" <%# Eval("DS_MOTIVO_ACAO") %> >Divergência Indenização</option>
                                <option value="CancelamentoDeSeguro" <%# Eval("DS_MOTIVO_ACAO") %> >Cancelamento de Seguro</option>
                                <option value="DemoraNosReparos" <%# Eval("DS_MOTIVO_ACAO") %> >Demora nos Reparos</option>
                                <option value="VicioReincidente" <%# Eval("DS_MOTIVO_ACAO") %> >Vicio Reincidente</option>
                                <option value="DemoraNoPagamento" <%# Eval("DS_MOTIVO_ACAO") %> >Demora no Pagamento</option>
                                <option value="DemoraNaRegulacao" <%# Eval("DS_MOTIVO_ACAO") %> >Demora na Regulação</option>
                                <option value="FaltaDePrestaçãoDeInformacao" <%# Eval("DS_MOTIVO_ACAO") %> >Falta de Prestação de Informação</option>
                            </select>

    <p></p>
                            <label>Réu </label>
                            <select class="novoSelect" id="cboReu" style="width:243px; height:69px; margin-top:0" size="7" >
                                <%# Eval("DS_REU") %>
                            </select>

                            <input id="txtReu" type="texto" style="width:238px" />
                            <form action="" method="post">
                                <label id="input_adicional_reu"></label>
                                <label><input type="button" name="addReu" value="+" /></label>
                            </form>
    <p></p>
                         
                            <label>Objeto </label>
                            <input id="txtObjeto" type="text" value='<%# Eval("DS_OBJETO") %>' style="width:238px" />
    <p></p>
    ...
    <p></p>
                    ...
                    <script type="text/javascript">
                        $(document).ready(function () {
                            $('#txtValorDoRisco, #cboProbabilidade').blur(function () {
                                var risco = $('#txtValorDoRisco').val();
                                risco = risco.replace(",", ".");
                                if (risco == "") risco = 0;
                                risco = parseFloat(risco);

                                var honorario = parseFloat("0");
                                honorario = risco * (10 / 100);
                                var honorarioFormatado = accounting.formatMoney(honorario, "", 2, ".", ",");
                                $('#txtNovoHonorario').val(honorarioFormatado);

                                var probabilidade = $('#cboProbabilidade').val();
                                
                                if (probabilidade == "3") var indenizacao = parseFloat(risco);
                                else if (probabilidade == "2") var indenizacao = parseFloat(risco / 2);
                                else var indenizacao =  parseFloat("0");

                                var indenizacaoFormatado = accounting.formatMoney(indenizacao, "", 2, ".", ",");
                                $('#txtNovaIndenizacao').val(indenizacaoFormatado);

                                var resultado = parseFloat(indenizacao + honorario);
                                var resultadoFormatado = accounting.formatMoney(resultado, "", 2, ".", ",");
                                $('#txtTotalGeral').val(resultadoFormatado);
                            });
                        });
                    </script>
                            
                    <asp:FormView ID="TotalGeral" runat="server" DataSourceID="dsReservaPendente"  Width="100%">
                        <ItemTemplate>
                            <table>
                            <tr> 
                                <td>
                                    <p></p>
                                    <label>Nova Indenização </label>
                                    <input id="txtNovaIndenizacao" type="texto" value='<%# Eval("vl_indenizacao") %>' style="width:90px;" readonly="readonly"  />
                                </td>
                                <td>
                                    <p></p>
                                    <label>Novo Honorário </label>
                                    <input id="txtNovoHonorario" type="texto" value='<%# Eval("vl_honorario") %>' style="width:90px; margin-left:5px;" readonly="readonly"  />
                                </td>
                                <td>
                                    <p></p>
                                    <label>Total Geral </label>
                                    <input id="txtTotalGeral" type="texto" value='<%# Eval("vl_soma") %>' style="width:105px; margin-left:5px;" readonly="readonly"/>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:FormView>
                            ...
                            <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input id="txtDataCadastro" type="texto" value='<%# Eval("DT_CADASTRO") %>' style="width:90px" />
                                    </td>
                                    <td>
                                        <p></p>
    ...
    <p></p>
                            ...
                            <label>CPF do Autor</label>
                                <select class="novoSelect" id="cboAutor" style="width:310px; height:69px; margin-top:0" size="7" >
                                    <%# Eval("DS_AUTOR") %>
                                </select>
                                <input id="txtAutor" type="texto" style="width:305px" />
                                <form action="" method="post">
                                    <label id="input_adicional_autor"></label>
                                    <label><input type="button" name="addAutor" value="+" /></label>
                                </form>
                                <p></p>
    ...
    <p></p>
                                ...
                                <script type="text/javascript">
                                    $(function () {
                                        for (var k = 1; k <= 100; k++)
                                            $('#cboVaraTeste').append("<option value='" + k + "'>" + k + "</option>");
                                    });
                                </script>
                                <label for="source">Vara</label>
                                <select id="cboVaraTeste" class="novoSelect" style="width:243px; height:21px; margin-top:0">
    ...
    <p></p>
                    ...
                    <div class="button-bar-inline-right" id="Div6">
                        ...
                        <button type="button" id="btnCadastroPessoaJudicial" class="btnCadastroPessoa ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" >Cadastro de Pessoas</button>
                    </div>
...
<div class="both"></div>
        ...
        $(document).ready(function () {
            ...
            .click(function () {
                $.ajax({
                    type: "get",
                    contentType: "application/json",
                    url: "pessoa?numeroCPF=" + document.getElementById('txtCPFPessoa').value,
                    data: "{}",
                    dataType: 'json',
                    success: function (result) {
                        //add data
                        ...
                        for (i = 0; i < result.length; i++) {
                            var item = result[i];
                            rows.push({ cell: [item.TP_DOCTO, item.NR_DOCTO, item.DS_PESSOA, ''] });
                        }
                        pessoas = { total: result.length, page: 1, rows: rows }
            ...
            .click(function () {
                $.ajax({
                    type: "get",
                    url: "pessoa?numeroConta=" + document.getElementById('txtConta').value + '$' +
                                                 document.getElementById('txtAgencia').value + '$' +
                                                 document.getElementById('cboBanco').value,
                    success: function (result) {
                        var rows = Array();

                        for (i = 0; i < result.length; i++) {
                            var item = result[i];
                            rows.push({ cell: [item.TP_CONTA, item.NRO_CONTA, item.DV_CONTA, item.DS_BANCO, item.CD_AGENCIA, item.DV_AGENCIA, item.FL_CONTA_PRINCIPAL, item.DS_PESSOA, ''] });
                        }
                        pessoas = { total: result.length, page: 1, rows: rows }
            ...
            .click(function () {
                ...
                if (!validarCPF(document.getElementById('txtCPFPessoaNovo').value)) {
                    if (!validarCNPJ(document.getElementById('txtCPFPessoaNovo').value)) {
                        alert('CPF ou CNPJ Inválido!');
                        return;
                    }
                }
                ...
                $.ajax({
                    type: "post",
                    url: "pessoa?doctoNovo=" + document.getElementById('txtCPFPessoaNovo').value + '$' + document.getElementById('txtCPFPessoa').value,
                    error: function (xhr, status, error) {
                        var err = eval("(" + xhr.responseText + ")");
                        alert(err.ExceptionMessage);
                    },
                    success: function (result) {
                        alert('CPF / CNPJ alterado com sucesso.');
                        $.ajax({
                            type: "get",
                            contentType: "application/json",
                            url: "pessoa?numeroCPF=" + document.getElementById('txtCPFPessoaNovo').value,
                            data: "{}",
                            dataType: 'json',
                            success: function (result) {
                                //add data
            ...
            .click(function () {
                AlterarConta.style.display = 'block';
            });

            //Felipe Campos
            $('#btnConfirmarAlteracaoConta').button({
                icons: {
                    primary: "ui-icon-disk"
                },
                text: true
            ...
            .click(function () {
                $.ajax({
                    type: "post",
                    url: "pessoa?contaNova=" + document.getElementById('txtContaNovo').value + '$' + document.getElementById('txtContaDVNovo').value + '$' +
                                          document.getElementById('cboBancoNovo').value + '$' + document.getElementById('txtAgenciaNovo').value + '$' +
                                          document.getElementById('txtAgenciaDVNovo').value + '$' + document.getElementById('chkPrincipalNovo').value + '$' +
                                          document.getElementById('cboTipoContaNovo').value + '$' + document.getElementById('txtConta').value + '$' +
                                          document.getElementById('txtAgencia').value + '$' + document.getElementById('cboBanco').value,
                    error: function (xhr, status, error) {
                        var err = eval("(" + xhr.responseText + ")");
                        alert(err.ExceptionMessage);
                    },
                    success: function (result) {
                        ...
                        $.ajax({
                            type: "get",
                            url: "pessoa?numeroConta=" + document.getElementById('txtContaNovo').value + '$' +
                                                         document.getElementById('txtAgenciaNovo').value + '$' +
                                                         document.getElementById('cboBancoNovo').value,
                            success: function (result) {
                                var rows = Array();
    ...
    <div id="Pessoa" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable" style="outline: 0px none; z-index: 1003; height: 600px; width: 960px; top: 100px; left: 260px; display: none;" tabindex="-1" role="dialog" aria-labelledby="ui-id-7">
        ...
        <div id="Div2" class="ui-dialog-content ui-widget-content" style="display: block; width: auto; min-height: 0px; height: 600px;" scrolltop="0" scrollleft="0">
            ...
            <div class="button-bar-inline">
                <button type="button" id="btnPesquisarCPF" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" >Pesquisar</button>
            ...
            <!-- Felipe Campos -->
            <div class="cell-wizard" style="margin-left:100px;">
                <label for="Itens_DsObjSeguradoEdit"> CPF/CNPJ NOVO</label>
                <input id="txtCPFPessoaNovo" style="width: 100px;" type="text" />
            </div>

            <div class="button-bar-inline">
                <button type="button" id="btnAlterarCPF" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" >Alterar</button>
            </div>
            ...
            <div class="button-bar-inline">
                <button type="button" id="btnPesquisarConta" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Pesquisar</button>
                ...
                <button type="button" id="btnAlterarConta" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Alteração</button>
            </div>

            <!-- Felipe Campos -->
            <div id="AlterarConta" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable" style="outline: 0px none; z-index: 1003; height: 48px; width: 932px; top: 520px; left: 10px; display: none;" tabindex="-1" role="dialog" aria-labelledby="ui-id-7">
                <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> Tipo da Conta Novo</label>
                <select id="cboTipoContaNovo" style="width: 100px;" >
                  <option value="1">Corrente</option>
                  <option value="2">Poupança</option>
                </select>
            </div>
            ...
            <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> Banco Novo</label>
                <select id="cboBancoNovo" style="width: 200px;" >
                    <option value="0">000 - Banco Bankpar S.A.	</option>
                    <option value="1">001 - Banco do Brasil S.A.	</option>
                    <option value="3">003 - Banco da Amazônia S.A.	</option>
                    <option value="4">004 - Banco do Nordeste do Brasil S.A.	</option>
            ...
            <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> Agencia Novo</label>
                <input id="txtAgenciaNovo" style="width: 50px;" type="text" />
            </div>

            <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> DV Novo</label>
                <input id="txtAgenciaDVNovo" style="width: 20px;" type="text" />
            </div>

            <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> Número Novo</label>
                <input id="txtContaNovo" style="width: 50px;" type="text" />
            </div>

            <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> DV Novo</label>
                <input id="txtContaDVNovo" style="width: 20px;" type="text" />
            </div>

            <div class="cell-wizard">
                <label for="Itens_DsObjSeguradoEdit"> Principal Novo</label>
                <input id="chkPrincipalNovo" type="checkbox" />
            </div>

            <div class="button-bar-inline">
                <button type="button" id="btnConfirmarAlteracaoConta" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Confirmar Novos Dados</button>
                <button type="button" id="btnLimparContaNova" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary">Cancelar</button>
            </div>
            </div>
    ...
    <div id="beneficiarios" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable" style="outline: 0px none; z-index: 1002; height: 600px; width: 695px; top: 173px; left: 438.5px; display: none;" tabindex="-1" role="dialog" aria-labelledby="ui-id-7">
        ...
        <div id="editDialogItem" class="ui-dialog-content ui-widget-content" style="display: block; width: auto; min-height: 0px; height: 550px;" scrolltop="0" scrollleft="0">
            ...
            <div class="button-bar-inline-right">
                ...
                <button type="button" id="btnCadastroPessoa" class="btnCadastroPessoa ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" >Cadastro de Pessoas</button>
